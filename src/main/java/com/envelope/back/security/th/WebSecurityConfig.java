...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                // 1. 配置不拦截路由
                // 404,403,500 所有异常页面不排除
                // toLogin 登录页面不拦截
                // getCode 二维码接口不拦截
                .antMatchers("/404", "/403", "/500", "/toLogin", "/getCode").permitAll()
                // 任何其它请求
                .anyRequest()
                // 都需要身份认证
                .authenticated()
                ...
                // 只要保持表单中action和HttpSecurity里配置的loginProcessingUrl一致就可以了，也不用自己去处理，它不会将请求传递给Spring MVC和您的控制器，所以我们就不需要自己再去写一个/envelope/login的控制器接口了
